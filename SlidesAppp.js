/**
 * Internal Utility Class: SlidesAppp
 * Refactored to ES6 Class for ImgApp v2.0.0
 */

class SlidesAppp {
  constructor(id_ = "create") {
    this._checkAPIs();
    this.name = "SlidesAppp";
    if (id_ !== "create") {
      if (
        !id_ ||
        DriveApp.getFileById(id_).getMimeType() !== MimeType.GOOGLE_SLIDES
      ) {
        throw new Error("This file ID is not the file ID of Google Slides.");
      }
      this.obj = { presentationId: id_ };
    }
  }

  _checkAPIs() {
    if (typeof Drive === "undefined" || typeof Slides === "undefined") {
      throw new Error(
        "Please enable Drive API and Slides API at Advanced Google services.",
      );
    }
  }

  createNewSlidesWithPageSize(obj_) {
    if (!obj_ || Object.keys(obj_).length === 0) {
      throw new Error("Object was not found. Please confirm it again.");
    }

    const pptxObj = this._newPPTXdata();
    this._setPageSize(obj_, pptxObj);
    const blob = this._pptxObjToBlob(pptxObj);

    const createObj = {
      name: obj_.title || "SlidesSample",
      mimeType: MimeType.GOOGLE_SLIDES,
    };

    if (obj_.hasOwnProperty("parent")) {
      createObj.parents = [obj_.parent];
    }

    try {
      return Drive.Files.create(createObj, blob).id;
    } catch (e) {
      if (e.message.includes("Drive is not defined")) {
        throw new Error(
          "Please enable Drive API at Advanced Google services, and try again.",
        );
      } else {
        throw new Error(e.message);
      }
    }
  }

  _newPPTXdata() {
    const data =
      "";
    return JSON.parse(
      Utilities.unzip(
        Utilities.newBlob(Utilities.base64Decode(data), MimeType.ZIP),
      )[0].getDataAsString(),
    );
  }

  _setPageSize(obj_, obj) {
    if (obj_.hasOwnProperty("width") || obj_.hasOwnProperty("height")) {
      let unitX = obj_.width?.unit || "pixel";
      let unitY = obj_.height?.unit || "pixel";

      if (
        (unitX !== "pixel" && unitX !== "point") ||
        (unitY !== "pixel" && unitY !== "point")
      ) {
        throw new Error("Unit is wrong. Only 'pixel' or 'point' are allowed.");
      }
      if (
        !obj_.width.hasOwnProperty("size") ||
        !obj_.height.hasOwnProperty("size")
      ) {
        throw new Error("Size was not found in the configuration object.");
      }

      const w =
        (unitX === "pixel" ? obj_.width.size * 0.75 : obj_.width.size) * 12700;
      const h =
        (unitY === "pixel" ? obj_.height.size * 0.75 : obj_.height.size) *
        12700;

      const filename = "ppt/presentation.xml";
      const xmlObj = XmlService.parse(obj[filename]);
      const root = xmlObj.getRootElement();
      root
        .getChild("sldSz", root.getNamespace("p"))
        .setAttribute("cx", Math.floor(w))
        .setAttribute("cy", Math.floor(h));

      obj[filename] = XmlService.getRawFormat().format(root);
    }
  }

  _pptxObjToBlob(pptxObj) {
    const blobs = Object.keys(pptxObj).map((k) => {
      const v = pptxObj[k];
      return v.toString() === "Blob"
        ? v
        : Utilities.newBlob(v, MimeType.PLAIN_TEXT, k);
    });
    return Utilities.zip(blobs, "temp.pptx").setContentType(
      MimeType.MICROSOFT_POWERPOINT,
    );
  }
}
